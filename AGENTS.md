# Agent Guide — ep_invite_via_email

Invite a user to a pad on Etherpad.
...
`ep_invite_via_email` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_invite_via_email`:
