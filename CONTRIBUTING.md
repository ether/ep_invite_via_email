# Contributing to ep_invite_via_email

Thanks for helping improve `ep_invite_via_email` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_invite_via_email` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_invite_via_email from your local clone
pnpm run plugins i --path ../ep_invite_via_email

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_invite_via_email/` are picked up after a server restart.
...
* Etherpad version, ep_invite_via_email version, browser + OS
* Steps to reproduce
